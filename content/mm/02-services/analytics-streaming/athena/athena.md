...
- **Hub Links**: `[[mm/index]]` | `[[service-catalog]]` | `[[domain-2-data-store-management]]` | `[[domain-3-data-processing]]` | `[[s3]]`
...
Amazon Athena ၏ နောက်ကွယ်တွင် distributed SQL execution အတွက် **Presto / Trino** (Engine Version 3) ကို အသုံးပြုထားပြီး၊ centralized ဖြစ်ကာ Apache Hive နှင့် တွဲဖက်အသုံးပြုနိုင်သော (Hive-compatible) metadata layer အဖြစ် **[[glue-data-catalog]]** ကို အသုံးပြုသည်။
...
2. **Metadata Retrieval**: Athena query engine သည် table schema၊ serialization library (SerDe)၊ S3 prefix location များနှင့် partition metadata များကို ရယူရန် **[[glue-data-catalog]]** သို့ ဆက်သွယ်သည်။
3. **Security & Access Evaluation**: Athena သည် IAM permission များကို စစ်ဆေးပြီး **[[lake-formation]]** မှတစ်ဆင့် fine-grained access control policy များကို စစ်ဆေးအကဲဖြတ်သည် (column-level၊ row-level နှင့် cell-level security filter များကို အသုံးပြုဆောင်ရွက်စေခြင်း)။
...
| :--- | :--- | :--- | :--- |
| **Performance Optimization** | Parquet၊ Snappy နှင့် Partition Projection များကို အသုံးပြု၍ scan ပြုလုပ်ရမည့် data ကို လျှော့ချရန်နှင့် query speed ကို အမြင့်ဆုံးရရှိစေရန်။ | CSV/JSON မှ Parquet သို့ ပြောင်းလဲခြင်း၊ partition metadata lookup များ နှေးကွေးခြင်း။ | `[[athena-performance]]` |
| **ACID Transactions (Apache Iceberg)** | S3 ပေါ်တွင် time-travel queries များနှင့်အတူ row-level `UPDATE`၊ `DELETE` နှင့် `MERGE INTO` operation များကို ဆောင်ရွက်ရန်။ | GDPR / CCPA right-to-be-forgotten၊ S3 ပေါ်ရှိ concurrent writer များ။ | `[[athena-iceberg]]` |
| **Athena for Apache Spark** | စက္ကန့်ပိုင်းအတွင်း စတင်နိုင်သော (< 1 sec startup) Interactive PySpark analytics နှင့် Jupyter notebook များ။ | EMR/Glue cluster များကို စောင့်ဆိုင်းစရာမလိုဘဲ Interactive Python data exploration ပြုလုပ်ခြင်း။ | `[[athena-spark]]` |
| **Federated Queries** | AWS Lambda connector များကို အသုံးပြု၍ S3 မဟုတ်သော source များ (DynamoDB, RDS, CloudWatch) ကို နေရာမရွှေ့ဘဲ တိုက်ရိုက် (in place) query ပြုလုပ်ရန်။ | ETL မပြုလုပ်ဘဲ SQL query တစ်ခုတည်းဖြင့် S3 နှင့် DynamoDB တစ်လျှောက် cross-query ပြုလုပ်ခြင်း။ | `[[athena-federated-query]]` |
| **Workgroups & Governance** | Multi-tenant isolation ပြုလုပ်ခြင်း၊ query တစ်ခုချင်းစီအလိုက်နှင့် workgroup တစ်ခုလုံးအလိုက် data scan limit များ သတ်မှတ်ခြင်း၊ မဖြစ်မနေ encryption သုံးစေခြင်း။ | မလိုအပ်ဘဲ query ကုန်ကျစရိတ်များ မြင့်တက်လာခြင်းကို ကာကွယ်ခြင်း၊ team အလိုက် query history များကို ခွဲခြားထားခြင်း။ | `[[athena-workgroups]]` |
| **CTAS & UNLOAD Statements** | S3 dataset များကို transform ပြုလုပ်ရန်၊ partition ခွဲရန်နှင့် compress ပြုလုပ်ရန် SQL ကို အသုံးပြုသည့် lightweight serverless ETL။ | Pure SQL ကို အသုံးပြု၍ raw CSV မှ Parquet သို့ ပြောင်းလဲခြင်း၊ query result များကို export ထုတ်ခြင်း။ | `[[athena-ctas]]` |
